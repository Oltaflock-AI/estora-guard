...
-- =========================================
-- ESTORA GUARD — AGENT LAYER MIGRATIONS
-- =========================================
-- Run these after the base schema above.
-- Then regenerate types: npm run types:supabase
-- =========================================

-- Migration 001: agent_receipts
-- Every agent action is logged here — allowed, denied, or pending approval.
...
-- Migration 002: RLS for agent_receipts

ALTER TABLE agent_receipts ENABLE ROW LEVEL SECURITY;

CREATE POLICY "users can read own receipts"
  ON agent_receipts FOR SELECT
  USING (
    user_id = auth.uid()
    OR EXISTS (
...
CREATE POLICY "service role can insert receipts"
  ON agent_receipts FOR INSERT WITH CHECK (true);

CREATE POLICY "service role can approve receipts"
  ON agent_receipts FOR UPDATE USING (true) WITH CHECK (true);

-- Migration 003: Scorecard view (used by Scoreboard component)
